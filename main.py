...
def dct2(a):
	bsize = 8
	diff = np.abs(np.diff(a, axis=0)) + np.abs(np.diff(a, axis=1))
	diffsum = np.zeros_like(a)
	for i in range(0, a.shape[0], bsize):
		for j in range(0, a.shape[1], bsize):
			diffsum[i:i+bsize, j:j+bsize] = np.sum(diff[i:i+bsize, j:j+bsize])
    # use the mean of the differences to determine the block size
	mean_diff = np.mean(diffsum)
	if mean_diff <= 20:
		bsize = 32
	elif mean_diff <= 40:
		bsize = 16
	...
	for i in range(0, a.shape[0], bsize):
		for j in range(0, a.shape[1], bsize):
			block = a[i:i+bsize, j:j+bsize]
			result[i:i+bsize, j:j+bsize] = cv.dct(block)
	return result
	# return cv.dct(a)
...
def idct(x, norm=None):
    ...
    if norm is not None:
        ...
        if norm == 'ortho':
            x[0] /= np.sqrt(2)
            w = np.exp(1j * np.pi / (2 * n) * np.arange(0.5, n))
            x = np.real(w * x)
        else:
            x *= np.sqrt(2 * n)
...
if __name__ == "__main__":
	...
	# wx = cv.imread("Extracted_SigAtt_GaussNoise.jpg")
	print("NCC:", NCC(wm, wx))
